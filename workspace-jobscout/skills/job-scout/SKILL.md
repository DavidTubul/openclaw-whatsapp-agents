...
triggers:
  - cron: "0 8 * * *"
    ...
    prompt: "scout"
  - channel: whatsapp
    group_id_ref: workspace-jobscout/.config/job-scout.json#whatsapp.group_id
    mode: conversational
tools:
  - browser
  - tavily
...
1. If message is exactly `scout` (from cron) OR `/scout` (from WhatsApp) → load `prompt-scout.md` and execute the daily scout pipeline (loops over enabled people in the registry).
...
1. **Never send WhatsApp messages outside the configured group** (`workspace-jobscout/.config/job-scout.json#whatsapp.group_id`). All outbound goes there.
...
| Tavily job search | `node /home/davidtobol2580/open_claw/workspace-jobscout/tools/search.mjs --person <id>` (Israeli boards + Indeed/Glassdoor, location-filtered) |
| LinkedIn job search | `node /home/davidtobol2580/open_claw/workspace-jobscout/tools/linkedin.mjs --person <id>` (free guest endpoint, no auth; backfill→incremental via `people/<id>/data/linkedin-seen.json`) |
| Telegram channel job fetch | `node /home/davidtobol2580/open_claw/workspace-jobscout/tools/telegram.mjs fetch --person <id>` (gramjs/MTProto, location-filtered; owner-only) |
| Gmail (read-only, owner-only, INCREMENTAL) | `node /home/davidtobol2580/open_claw/workspace-jobscout/tools/gmail-search.mjs --person <id> [--after-uid N]` (incremental via `people/<id>/data/gmail-state.json`) |
| Per-person sent-jobs ledger | `node /home/davidtobol2580/open_claw/workspace-jobscout/tools/ledger.mjs <person> <check|add>` — per-person dedup of jobs already pushed. **The ONLY dedup memory for guests** (who have no Sheet). |
| Google Sheet tracker (owner-only) | `node /home/davidtobol2580/open_claw/workspace-jobscout/tools/sheet.mjs <ping\|ids\|read\|append\|update-by-id\|update\|find\|sort>` — **prefer `update-by-id`** (id-addressed, immune to the auto-re-sort that scrambles row numbers); `update <row>` is legacy/fragile. |
| Content dedup key | `node /home/davidtobol2580/open_claw/workspace-jobscout/tools/jobkey.mjs "<company>" "<role>"` (→ stable 12-char id for company+role dedup) |
| Ad-hoc web search | `/home/davidtobol2580/open_claw/openclaw infer web search --provider tavily --query "..." --limit N --json` |
| Send WhatsApp (PROACTIVE pushes ONLY — e.g. the 08:00 scout report) | `/home/davidtobol2580/open_claw/openclaw message send --channel whatsapp --target "<group_id>" --message "..."`. **Do NOT use this to answer a message in conversation** — your final reply text is delivered to the group automatically. Calling `message send` while also replying produces a DUPLICATE message. |
| CV matching | YOUR own reasoning (you are the LLM) against the person's `workspace-jobscout/people/<id>/profile/cv-summary.json` |

Capabilities are gated per person by `people.json` → `people[].capabilities.{sheet,gmail,telegram}` (owner = all true; guests = all false). The shared `group_id`, `sheet_webhook_url`, and `sheet_url` live in `workspace-jobscout/.config/job-scout.json`; the person registry lives in `workspace-jobscout/.config/people.json` — read both first.
