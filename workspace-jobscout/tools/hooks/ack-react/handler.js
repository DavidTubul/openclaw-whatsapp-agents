...
export function decideReaction(event, groupId) {
  ...
  // A GROUP reaction key must match the original message exactly or WhatsApp accepts it
  // ("Sent reaction") but never renders it. Two parts:
  //  - participant: the sender's JID. NOTE `ctx.from` is the GROUP jid here, not the sender,
  //    so we take it from metadata.senderId / senderE164.
  //  - fromMe: this bot runs ON David's own WhatsApp account (account number == sender
  //    number), so David's own messages are `fromMe: true`. We must set that, else the key
  //    (which defaults fromMe:false) won't match his message.
...
export default async function ackReact(event) {
  try {
    ...
    const debugPath = "/tmp/ack-react-debug.json";
    await import("node:fs/promises").then(fs => fs.writeFile(debugPath,
