...
test("classify buckets scout reports, resets, internal logs, and chat", () => {
  assert.equal(classify({ from: "scotty", text: "🔵 בוקר טוב David! — 07/06/2026\n📊 סטטוס" }), "scout");
  assert.equal(classify({ from: "scotty", text: "@972500000000 🔵 בוקר טוב אורח! — 07/06/2026" }), "scout");
  assert.equal(classify({ from: "scotty", text: "Daily scout complete — both people processed" }), "internal");
  assert.equal(classify({ from: "scotty", text: "התחלתי שיחה חדשה כדי להישאר חד 🙂" }), "reset");
  assert.equal(classify({ from: "david", text: "היי סקוטי מה קורה" }), "chat");
  assert.equal(classify({ from: "scotty", text: "בוקר אור David! ☀️ מה שלומך?" }), "chat"); // a real conversational reply
});

test("formatRecentMd collapses noise runs to one marker and keeps N chat turns", () => {
  const recs = [
    { ts: "2026-06-01T18:00:00Z", from: "david", text: "ערב טוב" },
    { ts: "2026-06-01T18:01:00Z", from: "scotty", text: "ערב טוב David!" },
    // a morning's worth of noise — should collapse to ONE marker, not 3 slots
    { ts: "2026-06-02T04:31:00Z", from: "scotty", text: "התחלתי שיחה חדשה כדי להישאר חד" },
    { ts: "2026-06-02T05:05:00Z", from: "scotty", text: "🔵 בוקר טוב David! — 02/06/2026\n📊 סטטוס" },
    { ts: "2026-06-02T05:08:00Z", from: "scotty", text: "Daily scout complete — both people processed" },
  ...
  // the noise is collapsed to a single dated marker, not three full lines
  assert.ok(md.includes("סריקה יומית · דוח נשלח"));
  assert.ok(!md.includes("📊 סטטוס"));
  assert.ok(!md.includes("Daily scout complete"));
...
test("formatRecentMd keeps only the last N chat turns, markers ride free", () => {
  const recs = [
    ...
    { ts: "2026-06-01T10:01:00Z", from: "scotty", text: "🔵 בוקר טוב — noise" },
...
test('decideLog captures senderE164 and fromMe=false for a guest received group msg', () => {
...
test('decideLog leaves messageId undefined for outbound (scotty) messages', () => {
  const d = decideLog(ev("sent", { content: "מצאתי 3 משרות" }), GROUP);
