#!/usr/bin/env node
// Deterministic safety harness for Scotty's chat-driven self-extension.
//
...
// made deterministic here, so a bad self-edit can never silently break the 08:00 cron.
//
// Flow the agent follows (see skills/job-scout/prompt-self-extend.md):
//   1. node self-edit.mjs snapshot '["skills/job-scout/prompt-qa.md", ...]'  -> {snapshot_id}
...
const TOOLS_DIR = dirname(fileURLToPath(import.meta.url));        // .../workspace/tools
const WORKSPACE = resolve(TOOLS_DIR, '..');                       // .../workspace
...
// What `verify` is allowed to touch / check. Adding a new tool that needs guarding?
// it is syntax-checked automatically (every *.mjs under tools/). Adding a new config
// that must stay valid JSON? list it here.
const GUARDED_JSON = [
  '.config/people.json',
  '.config/job-scout.json',
];
...
function cmdVerify(testsOnly) {
  ...
  // 1. The unit test suite — the core safety net (40+ tests over the lib helpers).
  // SELF_EDIT_VERIFYING tells self-edit.test.mjs's own verify-test to skip, so a verify
...
function listMjs() {
  return readdirSync(TOOLS_DIR).filter((f) => f.endsWith('.mjs')).map((f) => join(TOOLS_DIR, f))
    .concat(readdirSync(join(TOOLS_DIR, 'lib')).filter((f) => f.endsWith('.mjs')).map((f) => join(TOOLS_DIR, 'lib', f)));
}
function listTestFiles() {
  // Cover BOTH tools/*.test.mjs (ledger, jobkey, session-hygiene, weekly-review) and
  // tools/lib/*.test.mjs (linkedin, location-filter, people, self-edit) — the safety net is
  // only as strong as the tests it runs.
  const lib = join(TOOLS_DIR, 'lib');
  const root = readdirSync(TOOLS_DIR).filter((f) => f.endsWith('.test.mjs')).map((f) => join(TOOLS_DIR, f));
  const libTests = readdirSync(lib).filter((f) => f.endsWith('.test.mjs')).map((f) => join(lib, f));
