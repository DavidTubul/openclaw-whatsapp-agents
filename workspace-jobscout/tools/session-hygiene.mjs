#!/usr/bin/env node
// THIN SHIM → shared/lib/session-hygiene.mjs (agentId "main"). All logic + wiring live in the shared
...
const record = getAgent('main');
if (!record) { console.error('unknown agentId: main'); process.exit(2); }
runHygiene({
  record,
  dryRun: process.argv.includes('--dry-run'),
  forceReset: process.argv.includes('--force-reset'),
  forcePoisoned: process.argv.includes('--force-reset-poisoned'),
}).catch((e) => { console.error(`[session-hygiene-main] fatal: ${e?.message || e}`); process.exit(1); });
