...
const ROOT = "/home/davidtobol2580/open_claw/workspace-pitzuchim";
...
// Rewrite the whole ledger (used by update). Append-friendly format preserved.
...
async function pushToSheet(action, row) {
  const c = cfg();
  if (!c?.sheet?.enabled || !c?.sheet?.webhook_url) return { skipped: "sheet disabled" };
  try {
    const res = await fetch(c.sheet.webhook_url, {
      method: "POST",
